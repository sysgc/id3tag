"""Read and write audio file tags (mp3, flac, m4a).
...
returns — it never has to know that mp3 uses "ID3 frames", flac uses "Vorbis
comments", and m4a uses "MP4 atoms", each with its own field names.
...
"""Name of the custom ID3 "TXXX" frame we use to store a MusicBrainz recording ID on mp3 files."""

MB_ALBUM_FRAME = "MusicBrainz Album Id"
"""Name of the custom ID3 "TXXX" frame we use to store a MusicBrainz release (album) ID on mp3 files."""

MB_ARTIST_FRAME = "MusicBrainz Artist Id"
"""Name of the custom ID3 "TXXX" frame we use to store a MusicBrainz artist ID on mp3 files."""
...
def read_tags(path: str) -> dict:
    ...
    This looks at the file's extension (`.mp3`, `.flac`, or `.m4a`) and
    delegates to the matching format-specific reader below. That reader
    always returns a dict with the same set of keys, regardless of format,
    so the rest of the app never has to care which audio format it's
    dealing with.
